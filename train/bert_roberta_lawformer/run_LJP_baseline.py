...
def preprocess_label_list_multi_label(example):
    ...
    # Task1: relevant_article column: 1. evalute it as a list(pandas.to_csv() will change the dtype of label column)
    example['relevant_article'] = example['relevant_article'][1:-1].split(" ")
    # Task2: if is_single_classification, we don't change the label column, else we eavaluate it as a list
    example['accusation'] = example['accusation'][1:-1].split(" ")
...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            #! recover from normaliziation
            preds = np.exp(preds)
            # preds = np.power(10, preds)
            result["mse"] = np.mean((preds - p.label_ids)**2).item()
            result["log-distance"] = np.mean(np.abs(np.log(preds+1) - np.log(p.label_ids+1))).item()
            def S_score_func(distance):
                if distance <= 0.2:
                    return 1.0
                elif distance <= 0.4:
                    return 0.8
                    ...
                    return 0.0
            distance = np.abs(np.log(preds+1) - np.log(p.label_ids+1))
            result["S-score"] = np.mean(np.array(list(map(S_score_func, list(distance))))).item()
        elif is_multi_label:
